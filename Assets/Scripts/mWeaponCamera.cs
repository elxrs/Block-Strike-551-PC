...
public class mWeaponCamera : MonoBehaviour
{
	...
	public static void SetSkin(int weaponID, int skin)
	{
		...
		if (SaveLoadManager.GetFireStatCounter(weaponID, skin) > -1 && instance.Weapons[instance.SelectedWeapon].FireStat != null)
		{
			instance.Weapons[instance.SelectedWeapon].FireStat.SetActive(true);
			string text = SaveLoadManager.GetFireStatCounter(weaponID, skin).ToString("D6");
	...
	public static void ActivePrevSticker(int pos, int id)
	{
		...
		if (instance.PrevSticker != pos)
		{
			...
			DOTween.ToAlpha(() => atlas.color, delegate(Color x)
			{
				atlas.color = x;
