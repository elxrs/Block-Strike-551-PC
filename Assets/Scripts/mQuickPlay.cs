...
public class mQuickPlay : MonoBehaviour
{
	...
	private static mQuickPlay instance;
	...
	public void SetMaxPlayers(int[] list)
	{
		...
		for (int i = 0; i < instance.MaxPlayersList.Length; i++)
		{
	...
	private void SelectServer(string mode, string map, int maxPlayers)
	{
		...
		{
			if (string.IsNullOrEmpty(roomList[i].GetPassword()) && roomList[i].PlayerCount != roomList[i].MaxPlayers && roomList[i].GetMapHash() == 0 && (mode == "Any" || roomList[i].GetGameMode().ToString() == mode) && (map == Localization.Get("Any") || roomList[i].GetSceneName() == map || mode == "Any") && (maxPlayers == 0 || roomList[i].MaxPlayers == maxPlayers))
			{
				list.Add(roomList[i]);
			}
		}
		if (list.Count == 0)
		{
			...
			mPopUp.ShowPopup(Localization.Get("The server with the selected data was not found. You want to create your own server?"), Localization.Get("Search Server"), Localization.Get("No"), delegate
			{
				mPopUp.HideAll("Server");
