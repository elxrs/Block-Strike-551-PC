...
public class DataMap : MonoBehaviour
{
	...
	private void Check()
	{
		...
		if (pView == null)
		{
			...
			InvokeRepeating("UpdateActiveGroup", 0f, 1f / PhotonNetwork.sendRateOnSerialize);
			GameManager.GetController().PlayerInput.FPCamera.GetComponent<Camera>().farClipPlane = 120f;
			GameObject gameObject = new GameObject("Camera");
			gameObject.transform.SetParent(GameManager.GetController().PlayerInput.FPCamera.Transform);
