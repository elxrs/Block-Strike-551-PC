...
public class WeaponManager : MonoBehaviour
{
	...
	{
		MaxDamage = false;
		instance.SelectedKnife = SaveLoadManager.GetWeaponSelected(WeaponType.Knife);
		instance.SelectedPistol = SaveLoadManager.GetWeaponSelected(WeaponType.Pistol);
		instance.SelectedRifle = SaveLoadManager.GetWeaponSelected(WeaponType.Rifle);
	}
	...
	public static void SetSelectWeapon(int weaponID)
	{
		...
		{
			if (GameSettings.instance.Weapons[i].ID == weaponID)
			{
	...
	public static int GetMemberDamage(PlayerSkinMember member, int weaponID)
	{
		...
		{
			instance.CachedPlayerWeapons = GameManager.GetController().PlayerInput.PlayerWeapon;
		}
	...
	public static int GetRandomWeaponID(WeaponType type)
	{
		...
		for (int i = 0; i < GameSettings.instance.Weapons.Count; i++)
		{
			if (GameSettings.instance.Weapons[i].Type != type || GameSettings.instance.Weapons[i].Lock)
			{
				continue;
			}
			if ((bool)GameSettings.instance.Weapons[i].Secret)
	...
	public static int GetRandomWeaponID(bool rifle, bool pistol, bool knife, bool secret)
	{
		...
		for (int i = 0; i < GameSettings.instance.Weapons.Count; i++)
		{
			...
			if ((bool)GameSettings.instance.Weapons[i].Secret)
			{
				if (secret && SaveLoadManager.GetWeapon(GameSettings.instance.Weapons[i].ID))
				{
					list.Add(GameSettings.instance.Weapons[i]);
				}
				continue;
	...
	public static WeaponSkinData GetWeaponSkin(int weaponID, int skinID)
	{
		for (int i = 0; i < GameSettings.instance.Weapons.Count; i++)
		{
			...
			{
				if (GameSettings.instance.WeaponsStore[i].Skins[j].ID == skinID)
				{
	...
	public static StickerData GetStickerData(int id)
	{
		...
		{
			if (GameSettings.instance.Stickers[i].ID == id)
			{
