...
public class SpeedHackDetector2 : MonoBehaviour
{
	...
	private void Check()
	{
		Loom.RunAsync(delegate
		{
			while (true)
			{
				...
				else if (Math.Abs(Math.Abs((dateTime - DateTime.UtcNow).TotalSeconds) - timeOnStart) > threshold)
				{
	...
	{
		return (DateTime.Now - Process.GetCurrentProcess().StartTime).TotalSeconds > difference;
	}
}
