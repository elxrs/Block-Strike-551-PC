...
public class mCreateCustomServer : MonoBehaviour
{
	...
	public void Open()
	{
		mPanelManager.ShowPanel("CreateCustomServer", true);
	...
	{
		string[] mapsList = CustomMapManager.GetMapsList(SelectMode);
		SelectMapPopupList.Clear();
		for (int i = 0; i < mapsList.Length; i++)
		{
			SelectMapPopupList.AddItem(CustomMapManager.GetMapName(mapsList[i]), mapsList[i]);
		}
	...
	{
		if (SelectMapPopupList.items.Count > 0)
		{
			mPopUp.ShowPopup("Удалить карту: " + GetMap(), Localization.Get("Delete"), Localization.Get("No"), delegate
			{
				mPopUp.HideAll();
			}, Localization.Get("Yes"), delegate
			{
				CustomMapManager.DeleteMap(GetMap());
				Open();
				mPopUp.HideAll();
			});
		}
	...
	public void CreateServer()
	{
		if (SelectMapPopupList.items.Count > 0)
		{
			CustomMapManager.Load((string)SelectMapPopupList.data);
			mPhotonSettings.OnCreateCustomServer();
		}
	}
}
