...
public class mAccountInfo : MonoBehaviour
{
	public static void Convert(AccountData data)
	{
		...
		stringBuilder.AppendLine("==========================================================");
		stringBuilder.AppendLine(Localization.Get("Time") + ": " + data.Time);
		stringBuilder.AppendLine("==========================================================");
		...
		stringBuilder.Append(Localization.Get("Head") + ": " + (data.PlayerSkin.Select[0] + 1));
		...
		stringBuilder.Append(Localization.Get("Body") + ": " + (data.PlayerSkin.Select[1] + 1));
		...
		stringBuilder.Append(Localization.Get("Legs") + ": " + (data.PlayerSkin.Select[2] + 1));
		...
		for (int m = 0; m < data.Weapons.Count; m++)
		{
			...
			for (int num = 0; num < data.Weapons[m].Skins.Count; m++)
			{
				...
				stringBuilder.AppendLine(weaponSkin.Name + ((data.Weapons[m].FireStats[num] == -1) ? string.Empty : string.Concat(" [", data.Weapons[m].FireStats[num], "]")));
			}
			stringBuilder.AppendLine("////////////////");
		}
