...
public class PhotonTransformViewPositionControl
{
	...
	public Vector3 UpdatePosition(Vector3 currentPosition)
	{
		...
		case PhotonTransformViewPositionModel.InterpolateOptions.EstimatedSpeed:
			if (m_OldNetworkPositions.Count != 0)
			{
				float num = Vector3.Distance(m_NetworkPosition, GetOldestStoredNetworkPosition()) / m_OldNetworkPositions.Count * PhotonNetwork.sendRateOnSerialize;
	...
	public Vector3 GetExtrapolatedPositionOffset()
	{
		...
		{
			num += PhotonNetwork.GetPing() / 1000f;
		}
