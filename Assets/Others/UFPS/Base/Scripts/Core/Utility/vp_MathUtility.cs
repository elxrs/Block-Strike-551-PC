...
public static class vp_MathUtility
{
	...
	{
		value.x = ((!(Mathf.Abs(value.x) < epsilon)) ? value.x : (nValue.int0));
		value.y = ((!(Mathf.Abs(value.y) < epsilon)) ? value.y : (nValue.int0));
		value.z = ((!(Mathf.Abs(value.z) < epsilon)) ? value.z : (nValue.int0));
		return value;
	}

	public static float SnapToZero(float value, float epsilon = 0.0001f)
	{
		value = ((!(Mathf.Abs(value) < epsilon)) ? value : (nValue.int0));
		return value;
	}
