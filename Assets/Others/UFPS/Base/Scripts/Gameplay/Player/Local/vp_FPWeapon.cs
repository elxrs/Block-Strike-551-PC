...
public class vp_FPWeapon : vp_Component
{
	...
	public CryptoFloat BobInputVelocityScale = 1f;

	public CryptoFloat BobMaxInputVelocity = 100f;

	public CryptoBool BobRequireGroundContact = true;
	...
	{
		m_MouseMove.x = LookAxis.x / Delta;
		m_MouseMove.y = LookAxis.y / Delta;
	...
	protected virtual void UpdateZoom()
	{
		if (!(m_FinalZoomTime <= Time.time) && m_Wielded)
		{
			...
			float t = nValue.float1 - (m_FinalZoomTime - Time.time) / RenderingZoomDamping;
			if (m_WeaponCamera != null && vp_Utility.IsActive(m_WeaponCamera.gameObject))
			{
	...
	protected virtual void UpdateBob()
	{
		if (!(BobAmplitude == Vector4.zero) && !(BobRate == Vector4.zero))
		{
			...
			{
				m_BobSpeed = ((!BobRequireGroundContact || Controller.isGrounded) ? Controller.velocity.sqrMagnitude : 0f);
			}
	...
	protected virtual void UpdateSwaying()
	{
		...
		Vector3 vector = Transform.InverseTransformDirection(m_SwayVel / 60f);
		...
		m_PositionSpring.AddForce(new Vector3(vector.x * (PositionWalkSlide.x * nValue.float00016), 0f - Mathf.Abs(vector.x * (PositionWalkSlide.y * nValue.float00016)), (0f - vector.z) * (PositionWalkSlide.z * nValue.float00016)));
		m_RotationSpring.AddForce(new Vector3(0f - Mathf.Abs(vector.x * (RotationStrafeSway.x * nValue.float016)), 0f - vector.x * (RotationStrafeSway.y * nValue.float016), vector.x * (RotationStrafeSway.z * nValue.float016)));
	}
