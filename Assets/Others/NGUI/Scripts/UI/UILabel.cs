...
public class UILabel : UIWidget
{
	...
	public float effectiveSpacingY
	{
		get
		{
			return (!mUseFloatSpacing) ? (mSpacingY) : mFloatSpacingY;
		}
	}
	...
	public float effectiveSpacingX
	{
		get
		{
			return (!mUseFloatSpacing) ? (mSpacingX) : mFloatSpacingX;
		}
	}
	...
	protected void SetActiveFont(Font fnt)
	{
		...
		{
			int value;
			if (mFontUsage.TryGetValue(font, out value))
			{
...
#if UNITY_EDITOR
	[NonSerialized] bool mAllowProcessing = true;
	[NonSerialized] bool mUsingTTF = true;
	...
	private void ProcessText(bool legacyMode, bool full)
	{
		...
		if (mFinalFontSize > 0)
		{
			...
			while (num3 > 0)
			{
				...
				{
					mScale = num3 / mFinalFontSize;
					NGUIText.fontScale = ((!flag) ? (mFontSize / mFont.defaultSize * mScale) : mScale);
				}
				...
				if (mOverflow == Overflow.ResizeFreely)
				{
					...
					if (num != 1f)
					{
						mWidth = Mathf.RoundToInt(mWidth / num);
					}
					mHeight = Mathf.Max(minHeight, Mathf.RoundToInt(mCalculatedSize.y));
					if (num2 != 1f)
					{
						mHeight = Mathf.RoundToInt(mHeight / num2);
					}
					if ((mWidth & 1) == 1)
					{
						mWidth++;
	...
	public Vector2 ApplyOffset(BetterList<Vector3> verts, int start)
	{
		...
		float f2 = Mathf.Lerp(mHeight, 0f, vector.y) + Mathf.Lerp(mCalculatedSize.y - mHeight, 0f, vector.y);
		f = Mathf.Round(f);
		f2 = Mathf.Round(f2);
	...
	public void ApplyShadow(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, int start, int end, float x, float y)
	{
		...
		for (int i = start; i < end; i++)
		{
			...
			Color color4 = color;
			color4.a = color3.a / 255f * color.a;
	...
	public void SetCurrentSelection()
	{
		...
		{
			text = ((!UIScrollablePopupList.current.isLocalized) ? UIScrollablePopupList.current.value : Localization.Get(UIScrollablePopupList.current.value));
		}
	}
