...
public class UICamera : MonoBehaviour
{
	...
	public static KeyCode currentKey
	{
		...
		set
		{
			...
			{
				HideTooltip();
				if (mLastScheme != 0 && current != null && current.autoHideCursor)
				{
	...
	public static bool Raycast(Vector3 inPos)
	{
		for (int i = 0; i < list.size; i++)
		{
			...
			int layerMask = currentCamera.cullingMask & uICamera.eventReceiverMask;
	...
	public static void Notify(GameObject go, string funcName, object obj)
	{
		if (mNotifying <= 10)
		{
			...
			{
				mNotifying++;
				NGUIEvents.Dispatch(go, funcName, obj);
				mNotifying--;
			}
		}
	}
	...
	private void Start()
	{
		...
		if (fallThrough == null)
		{
			...
			else
			{
				...
				fallThrough = ((!(transform.parent != null)) ? gameObject : transform.parent.gameObject);
			}
		}
