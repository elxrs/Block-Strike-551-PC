...
public class UIPanel : UIRect
{
	...
	private Vector3 drawCallOffsetCache = Vector3.zero;
	...
	public Vector3 drawCallOffset
	{
		get
		{
			if (frame1 < Time.frameCount)
			{
				frame1 = Time.frameCount + 180;
				...
				drawCallOffsetCache = new Vector3((!flag) ? 0f : (0f - num2), (!flag2) ? 0f : num2);
			}
			return drawCallOffsetCache;
		}
	}
	...
	public override Vector3[] GetSides(Transform relativeTo)
	{
		if (mClipping != 0)
		{
			...
			Transform transform = cachedTransform;
			mSides[0] = transform.TransformPoint(num, y, 0f);
			mSides[1] = transform.TransformPoint(x, num4, 0f);
			mSides[2] = transform.TransformPoint(num3, y, 0f);
			mSides[3] = transform.TransformPoint(x, num2, 0f);
			if (relativeTo != null)
			{
				for (int i = 0; i < 4; i++)
				{
	...
	public override float CalculateFinalAlpha(int frameID)
	{
...
#endif
		{
			...
			{
				finalAlpha = ((!(parent != null)) ? mAlpha : (parent.CalculateFinalAlpha(frameID) * mAlpha));
			}
			else
			{
				finalAlpha = mAlpha;
	...
	protected override void OnEnable()
	{
		...
		mMatrixFrame = -1;
		OnStart();
		usedForUI = anchorCamera != null && mCam.orthographic;
		base.OnEnable();
		mMatrixFrame = -1;
	}
	...
	private void UpdateTransformMatrix()
	{
		...
		if (mMatrixFrame < Time.frameCount)
		{
			mMatrixFrame = Time.frameCount + 120;
	...
	protected override void OnAnchor()
	{
		...
		if (leftAnchor.target == bottomAnchor.target && leftAnchor.target == rightAnchor.target && leftAnchor.target == topAnchor.target)
		{
			...
			else
			{
				...
				num = vector2.x + leftAnchor.absolute;
				num3 = vector2.y + bottomAnchor.absolute;
				num2 = vector2.x + rightAnchor.absolute;
				num4 = vector2.y + topAnchor.absolute;
			}
		}
		else
		{
	...
	private void LateUpdate()
	{
		...
		for (int count2 = list.Count; j < count2; j++)
		{
			...
			{
				uIPanel.startingRenderQueue = rq;
				uIPanel.UpdateDrawCalls();
				rq += uIPanel.drawCalls.Count;
			}
			else if (uIPanel.renderQueue == RenderQueue.StartAt)
			{
				...
				{
					rq = Mathf.Max(rq, uIPanel.startingRenderQueue + uIPanel.drawCalls.Count);
				}
			}
			else
			{
				...
				if (uIPanel.drawCalls.Count != 0)
				{
					rq = Mathf.Max(rq, uIPanel.startingRenderQueue + 1);
				}
			}
		}
	}
	...
	private void UpdateDrawCalls()
	{
		...
		{
			drawCallClipRange.z = UIRoot.ScreenWidth * 0.5f;
		}
		if (drawCallClipRange.w == 0f)
		{
			drawCallClipRange.w = UIRoot.ScreenHeight * 0.5f;
		}
		if (halfPixelOffset)
		{
		...
		for (int i = 0; i < drawCalls.Count; i++)
		{
			...
			transform3.localScale = transform.lossyScale;
	...
	public void UpdateWidgetsAlpha(float alpha)
	{
	...
	private void UpdateWidgets()
	{
		...
		if (frame2 < Time.frameCount)
		{
			widgetCount = widgets.Count;
			frame2 = Time.frameCount + 180;
		}
...
#endif
				if (uIWidget.UpdateTransform(frame) || mResized)
				{
					bool visibleByAlpha = forceVisible || uIWidget.CalculateCumulativeAlpha(frame) > 0.001f;
					uIWidget.UpdateVisibility(visibleByAlpha, forceVisible || (!hasCumulativeClipping && !uIWidget.hideIfOffScreen) || IsVisible(uIWidget));
				}
				if (!uIWidget.UpdateGeometry(frame))
				{
					continue;
				}
				changed = true;
