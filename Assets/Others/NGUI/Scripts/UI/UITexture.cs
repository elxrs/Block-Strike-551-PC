...
public class UITexture : UIBasicSprite
{
	...
	public override void OnFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (!(texture == null))
		{
			Rect rect = new Rect(mRect.x * texture.width, mRect.y * texture.height, texture.width * mRect.width, texture.height * mRect.height);
			Rect inner = rect;
			...
			inner.xMax -= vector.z;
			inner.yMax -= vector.w;
			float num = 1f / texture.width;
			float num2 = 1f / texture.height;
			rect.xMin *= num;
			rect.xMax *= num;
			rect.yMin *= num2;
			rect.yMax *= num2;
