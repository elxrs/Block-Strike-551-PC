...
public class UIRoot : MonoBehaviour
{
	...
	public int activeHeight
	{
		get
		{
			if (activeScaling == Scaling.Flexible)
			{
				...
				if (screenSize.y < minimumHeight)
				{
					screenSize.y = minimumHeight;
					screenSize.x = screenSize.y * num;
				}
				else if (screenSize.y > maximumHeight)
				{
			...
			float num4 = manualWidth / manualHeight;
			switch (constraint)
			{
			case Constraint.FitWidth:
				return Mathf.RoundToInt(manualWidth / num3);
			case Constraint.Fit:
				return (!(num4 > num3)) ? manualHeight : Mathf.RoundToInt(manualWidth / num3);
	...
	public float GetPixelSizeAdjustment(int height)
	{
		...
		{
			return activeHeight / (float)height;
		}
