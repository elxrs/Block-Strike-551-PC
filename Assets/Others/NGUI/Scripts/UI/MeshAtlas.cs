...
public class MeshAtlas : MonoBehaviour
{
	...
	private void UpdateVertices()
	{
		if (!(atlasMesh == null))
		{
			...
			{
				vertices[i].x = (vertices[i].x + mPivot.x) * mScale.x * ((!mMirrorX) ? 1 : (-1));
				vertices[i].y = (vertices[i].y + mPivot.y) * mScale.y * ((!mMirrorY) ? 1 : (-1));
				vertices[i].z = (vertices[i].z + mPivot.z) * mScale.z * ((!mMirrorZ) ? 1 : (-1));
			}
