...
public abstract class UIBasicSprite : UIWidget
{
	...
	private void TiledFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		Vector2 vector = new Vector2(mInnerUV.width * texture.width, mInnerUV.height * texture.height);
		vector *= pixelSize;
		...
		if (mFillAmount < 1f)
		{
			...
			if (mFillDirection == FillDirection.Radial180)
			{
				for (int j = 0; j < 2; j++)
				{
					...
					float value = (mInvert ? (mFillAmount * 2f - (1 - j)) : (fillAmount * 2f - j));
			...
			if (mFillDirection == FillDirection.Radial360)
			{
				for (int l = 0; l < 4; l++)
				{
					...
					mTempUVs[2].y = mTempUVs[1].y;
					mTempUVs[3].y = mTempUVs[0].y;
					float value2 = ((!mInvert) ? (mFillAmount * 4f - (3 - NGUIMath.RepeatIndex(l + 2, 4))) : (mFillAmount * 4f - NGUIMath.RepeatIndex(l + 2, 4)));
	...
	private void AdvancedFill(BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		Vector4 vector2 = drawingDimensions;
		Vector2 vector3 = new Vector2(mInnerUV.width * texture.width, mInnerUV.height * texture.height);
		vector3 *= pixelSize;
		if (vector3.x < 1f)
		{
			vector3.x = 1f;
