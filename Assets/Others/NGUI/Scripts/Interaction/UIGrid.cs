...
public class UIGrid : UIWidgetContainer
{
	...
	public List<Transform> GetChildList()
	{
		...
		for (int i = 0; i < transform.childCount; i++)
		{
			...
			if (!hideInactive || (child && NGUITools.GetActive(child.gameObject)))
			{
				list.Add(child);
			}
		}
	...
	protected virtual void ResetPosition(List<Transform> list)
	{
		...
		for (int count = list.Count; i < count; i++)
		{
			...
			{
				vector = ((arrangement != 0) ? new Vector3(cellWidth * num2, (0f - cellHeight) * num, z) : new Vector3(cellWidth * num, (0f - cellHeight) * num2, z));
			}
		...
		{
			num5 = Mathf.Lerp(0f, num3 * cellWidth, pivotOffset.x);
			num6 = Mathf.Lerp(-num4 * cellHeight, 0f, pivotOffset.y);
		}
		else
		{
			num5 = Mathf.Lerp(0f, num4 * cellWidth, pivotOffset.x);
			num6 = Mathf.Lerp(-num3 * cellHeight, 0f, pivotOffset.y);
		}
