...
[ExecuteInEditMode]
public class UIScrollView : NGUIBehaviour
{
	...
	protected virtual bool shouldMove
	{
		get
		{
			...
			float num = ((finalClipRegion.z != 0f) ? (finalClipRegion.z * 0.5f) : (Screen.width));
			float num2 = ((finalClipRegion.w != 0f) ? (finalClipRegion.w * 0.5f) : (Screen.height));
	...
	public void Drag()
	{
		if (UICamera.currentScheme == UICamera.ControlScheme.Controller || !enabled || !NGUITools.GetActive(gameObject) || !mShouldMove)
		{
			return;
		}
		if (mDragID == -10)
	...
	private void LateUpdate()
	{
		...
		if (showScrollBars != 0 && (verticalScrollBar || horizontalScrollBar))
		{
			bool flag = false;
			bool flag2 = false;
