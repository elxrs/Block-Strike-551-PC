...
[ExecuteInEditMode]
public class UIPopupList : NGUIBehaviour
{
	...
	public override void OnNavigate(KeyCode key)
	{
	...
	{
		if (enabled && current == this && (key == UICamera.current.cancelKey0 || key == UICamera.current.cancelKey1))
		{
			OnSelect(false);
		}
	}
	...
	private IEnumerator CloseIfUnselected()
	{
		...
		{
			yield return null;
			if (!(UICamera.selectedObject != mSelection) || !(UICamera.selectedObject != mBackground.cachedGameObject) || !(UICamera.selectedObject != mHighlight.cachedGameObject) || !(UICamera.selectedObject != mHighlightedLabel.cachedGameObject))
			{
				continue;
			}
			bool equal = false;
			...
			{
				if (UICamera.selectedObject == mLabelList[i].cachedGameObject)
				{
					equal = true;
					break;
				}
	...
	public virtual void Show()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && atlas != null && isValid && items.Count > 0)
		{
			...
			mHighlight = NGUITools.AddSprite(uIScrollView.gameObject, atlas, highlightSprite, mBackground.depth + 1);
			...
			{
				string text = items[i];
				UILabel uILabel = NGUITools.AddWidget<UILabel>(uIScrollView.gameObject, mBackground.depth + 2);
			...
			float f = a - (border.x + padding.x) * 2f + atlasSprite.borderLeft * num7;
			...
			if (flag)
			{
				...
				vector3.y = vector2.y + mBackground.height;
				vector3.x = vector2.x + mBackground.width;
				transform.localPosition = new Vector3(vector2.x, vector3.y - border.y, vector2.z);
			}
			else
			{
				vector3.y = vector2.y + border.y;
				vector2.y = vector3.y - mBackground.height;
				vector3.x = vector2.x + mBackground.width;
			}
