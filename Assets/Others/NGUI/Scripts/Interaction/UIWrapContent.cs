...
public class UIWrapContent : MonoBehaviour
{
	...
	{
		float num = (itemSize * mChildren.Count) * 0.5f;
		...
		if (mHorizontal)
		{
			float num3 = worldCorners[0].x - itemSize;
			float num4 = worldCorners[2].x + itemSize;
			int j = 0;
			for (int count = mChildren.Count; j < count; j++)
			{
				...
				if (num5 < 0f - num)
				{
					...
					int num6 = Mathf.RoundToInt(localPosition.x / itemSize);
				...
				else if (num5 > num)
				{
					...
					int num7 = Mathf.RoundToInt(localPosition2.x / itemSize);
		...
		{
			float num8 = worldCorners[0].y - itemSize;
			float num9 = worldCorners[2].y + itemSize;
			int k = 0;
			for (int count2 = mChildren.Count; k < count2; k++)
			{
				...
				if (num10 < 0f - num)
				{
					...
					int num11 = Mathf.RoundToInt(localPosition3.y / itemSize);
				...
				else if (num10 > num)
				{
					...
					int num12 = Mathf.RoundToInt(localPosition4.y / itemSize);
	...
	protected virtual void UpdateItem(Transform item, int index)
	{
		...
		{
			int realIndex = ((mScroll.movement != UIScrollView.Movement.Vertical) ? Mathf.RoundToInt(item.localPosition.x / itemSize) : Mathf.RoundToInt(item.localPosition.y / itemSize));
